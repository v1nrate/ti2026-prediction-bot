...
const ALIASES = {
  "Team Vision": ["Team Vision", "TEAM VISION", "Vision", "PARIVISION", "PARI VISION", "PVISION"],
...
const STATUS_ICON = { won: "✅", lost: "❌", waiting: "🟡", alive: "🟢" };
...
function pairKey(game) {
  return [game.radiant, game.dire].sort().join("|");
}
...
function buildSeries(games) {
  ...
  for (const cluster of clusters) {
    ...
    preliminary.push({
      key: `${cluster.pair}:${cluster.firstTime}`,
      startTime: cluster.firstTime,
      teamA: a,
      teamB: b,
      scoreA,
      scoreB,
...
function predictionStatus(kind, state) {
  ...
  if (kind === "4-0") {
    if (w === 4 && l === 0)
      return ["won", "точно закончили групповой этап 4-0"];
    if (l > 0 || w >= 4)
  ...
  if (kind === "4-1") {
    if (w === 4 && l === 1)
      return ["won", "точно закончили групповой этап 4-1"];
    if (l > 1 || (w >= 4 && l !== 1))
      return [
        "lost",
  ...
  if (kind === "1-4") {
    if (w === 1 && l === 4)
      return ["won", "точно закончили групповой этап 1-4"];
    if (w > 1 || (l >= 4 && w !== 1))
      return [
        "lost",
  ...
  if (kind === "0-4") {
    if (w === 0 && l === 4)
      return ["won", "точно закончили групповой этап 0-4"];
    if (w > 0 || l >= 4)
...
async function stratzQuery(env, query) {
  if (!env.STRATZ_TOKEN) {
    throw new Error("STRATZ_TOKEN secret is missing");
  }

  const response = await fetch(STRATZ_GRAPHQL, {
    method: "POST",
    headers: {
      "content-type": "application/json",
      accept: "application/json",
      authorization: `Bearer ${env.STRATZ_TOKEN}`,
      "user-agent": "TI2026PredictionBot/4.0",
    },
  ...
  try {
    ...
    throw new Error(`STRATZ returned invalid JSON: ${text.slice(0, 300)}`);
  }

  if (payload.errors?.length) {
    throw new Error(
      `STRATZ GraphQL: ${payload.errors
        .map((x) => x.message)
        .join(" | ")}`,
    );
  }
...
function parseStratzMatch(raw) {
  if (!raw?.id || Number(raw.leagueId) !== LEAGUE_ID) return null;

  const radiant = canonicalTeam(raw.radiantTeam?.name);
  const dire = canonicalTeam(raw.direTeam?.name);

  if (radiant === "Unknown" || dire === "Unknown") return null;

  const radiantWin = Boolean(raw.didRadiantWin);

  return {
    matchId: Number(raw.id),
    seriesId: null,
    startTime: Number(raw.startDateTime || 0),
    duration: 0,
    radiant,
    dire,
    radiantTeamId: Number(
      raw.radiantTeamId || raw.radiantTeam?.id || 0,
    ),
    direTeamId: Number(raw.direTeamId || raw.direTeam?.id || 0),
    radiantWin,
...
async function fetchTeamsMatches(env, teamIds) {
  const ids = [
  ...
  const query = `{
    teams(teamIds: [${ids.join(",")}]) {
      id
      name
      matches(request: { take: ${TEAM_MATCH_TAKE}, skip: 0 }) {
        id
        startDateTime
        leagueId
        radiantTeamId
...
async function fetchCurrentTIGames(env, knownIds) {
  const known = new Set(
    [...SEED_TEAM_IDS, ...(knownIds || [])].map(Number),
  );
  ...
  for (
    ...
    const teams = await fetchTeamsMatches(env, batch);
    const next = [];

    for (const team of teams) {
      for (const raw of team.matches || []) {
        if (Number(raw.leagueId) !== LEAGUE_ID) continue;

        const game = parseStratzMatch(raw);
        if (game) byId.set(game.matchId, game);

        for (const id of [raw.radiantTeamId, raw.direTeamId]) {
          const n = Number(id);
  ...
  return {
    games: [...byId.values()].sort(
      (a, b) =>
        a.startTime - b.startTime || a.matchId - b.matchId,
    ),
    teamIds: [...known],
  };
}
...
export class BotState extends DurableObject {
  ...
  async claimOwner(userId) {
    ...
    if (configured) {
      return Number(configured) === Number(userId);
    }
  ...
  async sync(notify = false) {
    ...
    const knownTeamIds =
      (await this.ctx.storage.get("knownTeamIds")) || SEED_TEAM_IDS;
    ...
    const byId = new Map(
      existing.map((g) => [Number(g.matchId), g]),
    );

    for (const g of incoming.games) {
      byId.set(Number(g.matchId), g);
    }
    ...
    return {
      ...
      newGames: Math.max(
        0,
        games.length - existing.length,
      ),
      knownTeams: incoming.teamIds.length,
  ...
  async handleTelegram(update) {
    ...
    if (text === "/status" || text === "📊 Статус") {
      ...
      if (!games.length) {
        const error =
          (await this.ctx.storage.get("lastError")) ||
    ...
    if (
      ...
      await sendTelegram(
        this.env,
        chatId,
        recentGamesText(await this.getGames()),
        telegramKeyboard(),
      );
  ...
  async fetch(request) {
    ...
    if (url.pathname === "/health") {
      ...
      return jsonResponse({
        ...
        games: games.length,
        knownTeams: (
          (await this.ctx.storage.get("knownTeamIds")) ||
          SEED_TEAM_IDS
        ).length,
        ownerId: await this.getOwnerId(),
        lastSync:
          (await this.ctx.storage.get("lastSync")) ||
          null,
        lastError:
          (await this.ctx.storage.get("lastError")) ||
          null,
  ...
  async fetch(request, env) {
    ...
    if (url.pathname === "/setup") {
      ...
      if (!env.STRATZ_TOKEN) {
        return jsonResponse(
          {
            ok: false,
            error: "STRATZ_TOKEN secret is missing",
          },
          500,
        );
      }
    ...
    return textResponse(
      "TI 2026 Prediction Bot is running.\n" +
        "Source: STRATZ.\n" +
        "Open /setup once after adding BOT_TOKEN and STRATZ_TOKEN.\n" +
